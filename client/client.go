...
func (c *Client) Hello() error {
	...
	if err := c.writeCommand("EHLO %s", c.config.LocalName); err != nil {
		return fmt.Errorf("sending EHLO command: %w", err)
	}
	...
	if err := c.writeCommand("HELO %s", c.config.LocalName); err != nil {
		return fmt.Errorf("sending HELO command: %w", err)
	}
...
func (c *Client) StartTLS() error {
	...
	if _, ok := c.extensions[ravenmail.ExtSTARTTLS]; !ok {
	...
	c.extensions = make(map[ravenmail.Extension]string)
	c.isESMTP = false
...
func (c *Client) Auth() error {
	...
	authExt, ok := c.extensions[ravenmail.ExtAuth]
	if !ok {
...
func (c *Client) authPLAIN() error {
	...
	if err := c.writeCommand("AUTH PLAIN %s", encoded); err != nil {
		return fmt.Errorf("sending AUTH PLAIN command: %w", err)
	}

	resp, err := c.readResponse()
	if err != nil {
		return fmt.Errorf("reading AUTH PLAIN response: %w", err)
	}
...
func (c *Client) authLOGIN() error {
	if err := c.writeCommand("AUTH LOGIN"); err != nil {
		return fmt.Errorf("sending AUTH LOGIN command: %w", err)
	}

	resp, err := c.readResponse()
	if err != nil {
		return fmt.Errorf("reading AUTH LOGIN challenge: %w", err)
	}
	...
	if err := c.writeCommand("%s", base64Encode([]byte(c.config.Auth.Username))); err != nil {
		return fmt.Errorf("sending AUTH LOGIN username: %w", err)
	}

	resp, err = c.readResponse()
	if err != nil {
		return fmt.Errorf("reading AUTH LOGIN username response: %w", err)
	}
	...
	if err := c.writeCommand("%s", base64Encode([]byte(c.config.Auth.Password))); err != nil {
		return fmt.Errorf("sending AUTH LOGIN password: %w", err)
	}

	resp, err = c.readResponse()
	if err != nil {
		return fmt.Errorf("reading AUTH LOGIN final response: %w", err)
	}
...
func (c *Client) Reset() error {
	...
	if err := c.writeCommand("RSET"); err != nil {
		return fmt.Errorf("sending RSET command: %w", err)
	}

	resp, err := c.readResponse()
	if err != nil {
		return fmt.Errorf("reading RSET response: %w", err)
	}
...
func (c *Client) Noop() error {
	...
	if err := c.writeCommand("NOOP"); err != nil {
		return fmt.Errorf("sending NOOP command: %w", err)
	}

	resp, err := c.readResponse()
	if err != nil {
		return fmt.Errorf("reading NOOP response: %w", err)
	}
...
func (c *Client) Quit() error {
	...
	if _, err := c.readResponse(); err != nil {
		// Some servers close immediately after QUIT; closing the socket still completes cleanup.
		_ = err
	}
...
func (c *Client) close() error {
	...
	if err != nil {
		return fmt.Errorf("closing SMTP connection: %w", err)
	}
	return nil
}
...
func (c *Client) parseExtensions(lines []string) {
	...
	for _, line := range lines[1:] { // Skip first line (greeting)
		...
		ext := ravenmail.Extension(strings.ToUpper(parts[0]))
		params := ""
		if len(parts) > 1 {
			params = parts[1]
		}
...
func (c *Client) writeCommand(format string, args ...any) error {
	...
	if c.config.WriteTimeout > 0 {
		if err := c.conn.SetWriteDeadline(time.Now().Add(c.config.WriteTimeout)); err != nil {
			return fmt.Errorf("setting write deadline for SMTP command %q: %w", cmd, err)
		}
	}

	_, err := c.writer.WriteString(cmd + "\r\n")
	if err != nil {
		return fmt.Errorf("writing SMTP command %q: %w", cmd, err)
	}

	if err := c.writer.Flush(); err != nil {
		return fmt.Errorf("flushing SMTP command %q: %w", cmd, err)
	}
	return nil
}
...
func (c *Client) readResponse() (*ClientResponse, error) {
	if c.config.ReadTimeout > 0 {
		if err := c.conn.SetReadDeadline(time.Now().Add(c.config.ReadTimeout)); err != nil {
			return nil, fmt.Errorf("setting read deadline for SMTP response: %w", err)
		}
	}
	...
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: invalid code in %q: %w", ErrUnexpectedResponse, line, err)
		}
